...
Este documento registra **deuda real aceptada**, abierta o resuelta, con evidencia concreta; las entradas resueltas se conservan por trazabilidad. No es un backlog de ideas, no convierte propuestas futuras en decisiones y no sustituye `WORK_ORDER_STATUS.md`. Cuando una deuda se programe, debe tener WO atómica propia; cuando se cierre, se conserva aquí como `RESOLVED` con referencia a la WO/PR.
...
| `FH-DEBT-002` / R2 `v6-sheet-lookup-split-brain` | RESOLVED | P2 robustez | RDD PR #370; `scripts/farmacia_common.js` detección v6 normalizada vs lookup físico en `parseWorkbook` | `isEnfermeriaV6Workbook()` reconoce nombres mediante normalización, pero `parseWorkbook()` recupera después las hojas con nombres canónicos exactos (`workbook.Sheets["DERMATOLOGÍA"]`, etc.). Un nombre físico equivalente para el detector puede no ser recuperable por el loader y producir rechazo incoherente. | Resolver una sola vez los nombres físicos reales a las 3 definiciones canónicas y reutilizar ese mapa en detección+parseo; test con variantes soportadas. | PR #374 → merge `771fb80c...`; candidate reconciliado `8f35a296...`; oracle `6dfd365...`; native review `review-bf84091b8c28d96d` APPROVED + authority burned |
| `FH-DEBT-003` / R3 `v6-sheet-name-normalization` | RESOLVED | P2 robustez | RDD PR #370; `enfermeriaV6Token()` + lookup v6 | La normalización elimina acentos, espacios y puntuación. Es útil, pero puede aceptar aliases demasiado permisivos o hacer ambiguas dos hojas físicas que colapsen al mismo token si no se valida unicidad. | Definir política cerrada de nombres/aliases; exigir exactamente una hoja física por servicio y rechazar aliases duplicados/ambiguos. Añadir tests de acentos, espacios, puntuación y doble alias. | PR #374 → merge `771fb80c...`; candidate reconciliado `8f35a296...`; oracle `6dfd365...`; native review `review-bf84091b8c28d96d` APPROVED + authority burned |
...
`FH-DEBT-002` + `FH-DEBT-003` quedaron resueltos y publicados por PR #374 con 718/718 comprobaciones deterministas verdes, CI smoke/Pages post-merge verdes y review nativo APPROVED/burned. `FH-DEBT-001` sigue abierto: requiere una decisión de persistencia/seguridad alineada con el entorno real de piloto y debe cerrarse antes de declarar el producto apto para piloto.
