...
| **WO-NEXUS-F3.2 (#403)** | PROMueve Nexus Home + native-v4-heavy field canary (retry) | 🟡 En PR | `work/nexus-home-f3-2-403-v4retry-20260925` | WU-A `172fb2b` (oracle congelado + bootstrap/shell, review `review-16e988d40fb1d7a6` APPROVED + burned); WU-B `17f9db6` (navegación same-tab + checker, review `review-98506c59b732ce1d` APPROVED + burned) | Determinista: `nexus_home_check` 11/11, `nexus_home_navigation_check` 11/11, `verify:nexus` PASS, `check:platform` PASS. Ficha técnica: [`NEXUS_HOME_F3.2.md`](../engineering/NEXUS_HOME_F3.2.md). Sin QA de navegador (F3.3), sin release (F3.4), no piloto/producción. Merge NO autorizado en la entrega. |
