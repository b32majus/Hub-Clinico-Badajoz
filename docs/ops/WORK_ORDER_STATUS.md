...
| **WO-NEXUS-F3.2 (#403)** | PROMueve Nexus Home + native-v4-heavy field canary (retry) | ✅ MERGED / functional gates verified; ⚠️ D011 OPEN | `work/nexus-home-f3-2-403-v4retry-20260925` | WU-A `172fb2b` (review `review-16e988d40fb1d7a6` APPROVED + burned); WU-B `17f9db6` (review `review-98506c59b732ce1d` APPROVED + burned); candidate `61e6e9c` → merge `e9096e9` (PR #404), tree `c346893...` idéntico | Pre-merge Home 11/11 + nav 11/11 + `verify:nexus` PASS. Post-merge fresh checkout: nav 11/11, `verify:nexus` PASS, Actions run `36116079829` success; Home oracle 9/11 por discrepancia **solo EOL** en los checks byte-identical de validators/manifest/readiness (`NEXUS-DEBT-011`, #407; contenido normalizado igual). Canary efectivo: probe + WU-A + WU-B en `nan/deepseek-v4-flash` high. #405 preserva D009/010. Sin QA browser (F3.3), sin F3.4, no piloto/producción. Ficha: [`NEXUS_HOME_F3.2.md`](../engineering/NEXUS_HOME_F3.2.md). |
| **WO-NEXUS-HOME-QUALIFICATION-03 (#409)** | Train de cualificación Home: F3.3 browser + F3.4 release sintético + cierre de deuda Nexus D008/D009/D010/D011 | 📋 Ready for review — candidata; sin merge | `work/nexus-home-qualification-03-409-20260925` | Q1 #410 `74f9963`; Q2 #411 `49deb61` (review `review-b11ff9bef660e52d` APPROVED + burned); Q3 #412 `b32ab74` (`review_due=false`); Q4 #413 `7c73f37` (review `review-36babcdade6b9454` APPROVED + burned); Q5a #414 `2407532`, Q5b #414 `612bc03` (reviews `review-d8422f1495f55e5b`, `review-268a65dbd0ee8825` APPROVED + burned) | Gates: fresh checkout Home 11/11 + nav 11/11 + `verify:nexus` PASS; `check:home-validators` 14/0; `check:home-release` 13/0; browser F3.3 8/0 y F3.4 5/0 sobre Chromium `151.0.7922.34`. Cierra `NEXUS-DEBT-008/009/010/011`; worker efectivo `nan/deepseek-v4-flash` high (`native-v4-heavy`). Findings informacionales no bloqueantes quedan como follow-ups (`R3-001`, `R3-malformed-request-crash`, `home_release_build.mjs:165-166`, `nexus_home_release_browser_check.mjs:245/274`). Evidencia sintética/determinista/browser-qualified en candidate; **no mergeada**; no piloto/producción; Cáceres/legacy intactos. Ficha: [`NEXUS_HOME_F3.4.md`](../engineering/NEXUS_HOME_F3.4.md). |
