...
| Reconciliación documental | 2026-08-03 — adaptadores; 2026-08-04 — Seguimiento; 2026-08-05 — raw reader y Quick View Bridge |
...
| 8A-2A-1 | `WO-FH-BRIDGE-V2-PATIENT-SELECTORS-QUICK-VIEW-01` | `MERGED_AND_VERIFIED` | Issue #241; PR #242; commits `3da3d450...` + `94cd4468...`; merge `e2c54583...`; Quick View dentro de `farmacia_index.html` |
| 8A-2A-2 | Handoff runtime + dashboard entre documentos HTML | **PENDIENTE** | No existe transporte del read model a otro HTML |
...
PR #231 retiró el ledger clínico basado en `localStorage` de las tres pantallas soportadas. PR #238 integra un Bridge raw v2 y PR #242 añade su consumidor UI Quick View; no usan `localStorage` ni `sessionStorage` y conservan el read model solo en memoria de la página actual. Imports legacy y snapshots anteriores pueden seguir usando `sessionStorage` como deuda separada; recarga o navegación completa exige volver a seleccionar el Excel.

PR #233 publicó el workbook operativo de Cáceres como contenedor del TSV: `01_DERMA` y `03_DIGESTIVO`, 152 columnas canónicas y 16 hojas técnicas ocultas y vacías. PR #238 añadió el lector raw v2 y read model; PR #242 conectó selectores y Quick View de lectura dentro de `farmacia_index.html`. Ambas hojas se leen, se valida el contrato y se preserva `1..N` sin aplanar pacientes. Esto no implementa handoff entre HTML, dashboard/formularios Bridge, Office Script, tablas relacionales pobladas, `APP_*`, Read Adapter ni roundtrip del Hub.
...
WO7 Office Script es un candidate publicado y pausado: debe superar el gate real de Microsoft Office Scripts antes de considerarse integrada. La lectura raw v2 ya está integrada independientemente por WO8A-1.

La Quick View de lectura en la misma página está integrada; quedan pendientes el handoff a otros HTML, dashboard/formularios Bridge, `APP_*`, descomposición relacional, Excel Read Adapter y roundtrip Hub → Excel → Hub. WO8A-2A-2, WO8A-2B y WO8A-3 no se declaran ejecutadas y solo podrán aprobarse como unidades futuras. Mantener el gate del paquete longitudinal final: no preparar URL, guía o paquete final ni retirar v1 hasta demostrar CIP arbitrario, Office Script, `APP_*`, Read Adapter y roundtrip completo.
