...
**Nexus Home Qualification 03 (TRAIN-NEXUS-HOME-QUALIFICATION-03, #409):** **CANDIDATA EN RAMA DE TRABAJO**, rama `work/nexus-home-qualification-03-409-20260925`, **sin PR ni merge**. Cerró `NEXUS-DEBT-011` y `NEXUS-DEBT-009` (Q1 #410 `74f9963`: LF canónico acotado por `.gitattributes` con detección byte-strict de drift preservada y cableado de `check:home`/`check:home-navigation` en `verify:nexus`), `NEXUS-DEBT-010` (Q2 #411 `49deb61`: gate fail-closed de allowlist de keywords en `home_validators_build.mjs` con `--schema-dir`, longitudes en code points y hardening checker 14/0; native review `APPROVED + burned` `review-b11ff9bef660e52d`) y `NEXUS-DEBT-008` (Q3 #412 `b32ab74`: el builder rechaza `entryPath` duplicado antes de emitir, checker 12/0, `review_due=false`). Además dejó F3.3 browser-qualified (Q4 #413 `7c73f37`: `nexus_home_f33_browser_check.mjs` 8/0 sobre Chromium real `151.0.7922.34` headless; native review `APPROVED + burned` `review-36babcdade6b9454`) y F3.4 con release sintético determinista 13/0 (Q5a #414 `2407532`) más cualificación browser del artefacto 5/0 (Q5b #414 `612bc03`; reviews `APPROVED + burned` `review-d8422f1495f55e5b` y `review-268a65dbd0ee8825`). Gates: fresh checkout Home 11/11 + navegación 11/11 + `verify:nexus` PASS; `check:home-validators` 14/0; `check:home-release` 13/0. Worker efectivo `nan/deepseek-v4-flash` high (`native-v4-heavy`). **Madurez:** gates deterministas + QA de navegador real + release sintético únicamente; snapshots Cáceres/legacy intactos; **no acredita** piloto ni producción. Detalle y rollback: [`engineering/NEXUS_HOME_F3.4.md`](engineering/NEXUS_HOME_F3.4.md).
