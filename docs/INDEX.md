...
| Rama de rescate integrada | `origin/rescue/farmacia-v4`. Último HEAD funcional reconciliado: `da35ced9fe00ffc3dc22cc938e83de72f5372db6` (PR #120). La propia reconciliación documental puede avanzar posteriormente el HEAD sin añadir funcionalidad clínica. |
...
| Rama de trabajo de este índice | `work/docs/fh-post-pr120-adherence-reconciliation-v4-20260726` |
...
| `origin/rescue/farmacia-v4` | Integración viva del rescate V4. Último HEAD funcional reconciliado: `da35ced9fe00ffc3dc22cc938e83de72f5372db6` (PR #120). La propia reconciliación documental puede avanzar posteriormente el HEAD sin añadir funcionalidad clínica. | S01–S08 cerrados en su alcance técnico previo; PR #114 integra contexto canónico y gate S09–S11 sobre `patient_id + line_id`; PR #116 integra borradores por línea y guard S12 dentro de Seguimiento; PR #120 integra captura estructurada de borrador de adherencia `mg1`–`mg4`, aislada por `patient_id + line_id`, con estados `ADHERENCE_EMPTY`, `ADHERENCE_PARTIAL` y `ADHERENCE_COMPLETE_UNINTERPRETED`, migración no destructiva v1→v2, fallo cerrado ante v2 corrupto/incompatible, estado dirty, guard S12 y QA soportada S09–S12. | Estado publicado, demo-ready, piloto, producción, datos reales, backend o V5; interpretación clínica de adherencia, registro clínico/asistencial, PROMs (incluidos DLQI/EVA), efectos adversos/causalidad, movimientos terapéuticos como optimización o suspensión, outputs JARA/CSV/Excel de Seguimiento, dashboards canónicos, cierre completo S09–S12 o guard S12 transversal. |
...
| [`docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md`](/docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md) | Plan vivo reconciliado hasta PR #120 | Gobierno y estado del rescate V4, referencias, fases, dependencias, aptitud y trabajo restante. | Autorización automática de código, publicación, piloto, backend o V5. |
...
| Seguimiento V4 S09–S12 | Fase iniciada parcialmente | Contexto/gate S09–S11, borradores/guard S12 y captura estructurada de adherencia completa pero no interpretada integrados; interpretación clínica, restantes capacidades y outputs pendientes. No están cerrados S09–S12 ni demostrado un guard S12 transversal fuera de Seguimiento. |
| Dashboards V4 | Pendientes posteriores | No están reconstruidos contra la fuente canónica. |
| Publicación, release y freeze V4 | Pendientes separados | El último HEAD funcional reconciliado no está publicado ni congelado. La fuente Pages y la candidata permanecen en `2ee3b347...` mediante PR #86; el estado actual no está acreditado como demo-ready ni como apto para piloto. |
...
*Índice generado en el contexto de `WO-DOC-INDEX-CONTROL-PLANE-POST-SES-01-20260713`, reconciliado previamente hasta PR #106 por `WO-DOC-FH-RESCUE-STATUS-RECONCILIATION-V4-01`, actualizado hasta PR #110 por `WO-DOC-FH-POST-PR110-STATUS-RECONCILIATION-V4-01`, reconciliado hasta PR #116 por `WO-DOC-FH-FOLLOWUP-POST-PR114-PR116-RECONCILIATION-V4-01` (issue #117) y hasta PR #120 por `WO-DOC-FH-POST-PR120-ADHERENCE-RECONCILIATION-V4-01` (issue #121). No autoriza cambios de código, publicación, merges ni integraciones.*
