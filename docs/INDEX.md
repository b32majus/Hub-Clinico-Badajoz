...
**Foundation Train 02 (TRAIN-NEXUS-FOUNDATION-02, #399):** **EJECUTADO — pendiente de merge autorizado. Atenea Promotion Review v1 inicial: FAIL (2026-09-25) sobre candidate `9e670bf`; dos correction work units aplicadas.** Siete work units (#394 EOL/provenance; #395 completitud manifest/readiness; #398 PlatformContext: ConfigurationRepository/EffectiveDeployment + facade; #396 oracle export Reuma 497: corpus+harness + acceptance/KNOWN_LEGACY ([`engineering/REUMA_EXPORT_KNOWN_LEGACY.md`](engineering/REUMA_EXPORT_KNOWN_LEGACY.md)); #397 consolidación CI en `verify:fast`/`verify:nexus` y workflow `nexus-checks.yml`) más dos correction WUs post-promotion-fail: **#398-C** `109b28d` (cross-artifact authority hardening: igualdad profile↔manifest en enabled/qualificationState, coherencia deploymentId/siteId/persistenceMode, identidad técnica registry↔manifest, 11 códigos nuevos, 13 negativos plantados) y **#396-C** `80fdd08` (gating acceptance-only: KNOWN_LEGACY/NON_GOLDEN report-only, nunca decide exit; corrección de un defecto legacy no puede romper `verify:nexus`). Los dos findings nonblocking de PlatformContext (procedencia de snapshots y test de aislamiento de input) se cerraron con las correction WUs **F3.1-D** `4bebeeb` (cierre de invariantes EffectiveDeployment: display/unicidades/rutas seguras/orden canónico/input cerrado/mapas seguros, 20 casos nuevos) y **F3.1-E** `4ee94c2` (trust boundary `SNAPSHOT_UNTRUSTED_ORIGIN` + aislamiento real del input original), dejando `NEXUS-DEBT-006` en RESOLVED. Base `66f1ddff7dea0ccf3ce84511bcff0f26694e7696`; rama `work/nexus-foundation-02-399-20260924`; candidate técnico `80fdd08` → correcciones F3.1-D/E `4bebeeb`/`4ee94c2`; PR #400 abierta contra `promueve/nexus-v4` (ver GitHub live). Antes de cualquier merge: nueva Atenea Promotion Review v1 fresca e independiente sobre el nuevo BASE/HEAD/diff exactos.
