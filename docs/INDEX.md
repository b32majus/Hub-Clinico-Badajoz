...
| Rama de rescate integrada | `origin/rescue/farmacia-v4`. Último HEAD funcional reconciliado: `2ac049ddb247a2784d2fb8a079700633230d8f79` (PR #132 / issue #131, completados). PR #130 / issue #129 completaron antes una reconciliación exclusivamente documental en `02b03e29901dbc63d92fb783db1593e82c9d8f3d`; no constituyó un HEAD funcional nuevo. |
...
| Rama de trabajo de este índice | `work/docs/fh-post-pr132-persisted-review-reconciliation-v4-20260726` |
...
| `origin/rescue/farmacia-v4` | Integración viva del rescate V4. Último HEAD funcional reconciliado: `2ac049ddb247a2784d2fb8a079700633230d8f79` (PR #132 / issue #131, completados). PR #130 / issue #129 fue la reconciliación documental posterior a PR #128 en `02b03e29901dbc63d92fb783db1593e82c9d8f3d`, no un HEAD funcional nuevo. | S01–S08 cerrados en su alcance técnico previo; PR #114 integra contexto/gate, PR #116 borradores/guard S12, PR #120 adherencia no interpretada, PR #124 EA crudo/no interpretado y PR #128 PROMs manuales/no interpretadas. PR #132 añade una tarjeta canónica de revisión de solo lectura del último borrador persistido exacto por `patient_id + line_id`, con estados `REVIEW_CONTEXT_BLOCKED`, `REVIEW_EMPTY`, `REVIEW_READY`, `REVIEW_STALE_UNSAVED_CHANGES` y `REVIEW_STORAGE_ERROR`. Lee exclusivamente `farmaciaDemo.followupDrafts.v4`, nunca los inputs dirty: si hay cambios dirty con persistido conserva los bytes guardados y muestra stale; sin persistido permanece `REVIEW_EMPTY`. PROM cero se renderiza `0` y la ausencia, `No informado`. Mantiene aislamiento exacto paciente/línea y falla cerrado ante storage corrupto, incompatible o no disponible. El evento `farmacia:followup-draft-state-v4` contiene solo `patient_id`, `line_id`, `ready`, `dirty`, `storage_error`, `has_saved` y `reason`, sin payload clínico. El store/esquema v4 no cambia, no existe segundo store y el módulo de revisión no escribe. S09–S12, `PATIENT_NOT_FOUND`, `PATIENT_MISMATCH` y misma identidad `blocked`→`active` quedaron demostrados mediante interacciones soportadas, con consola y `pageerror` a cero. | Estado publicado, demo-ready, piloto, producción, datos reales, backend o V5. La tarjeta no es un registro clínico/asistencial final ni firmado. Siguen pendientes interpretación de adherencia/EA/PROMs, cálculo DLQI, umbrales, clasificación, alertas, recomendaciones o tendencias; causalidad de EA, fármaco sospechoso, corrección aplicada o decisión terapéutica; optimización, switch, reducción de dosis o suspensión; outputs JARA/CSV/Excel de Seguimiento, dashboards, cierre clínico completo de Seguimiento/S09–S12 y guard S12 transversal fuera de Seguimiento. Los módulos legacy, causalidad, outputs y dashboards permanecen inertes. |
...
| [`docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md`](/docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md) | Plan vivo reconciliado hasta PR #132 | Gobierno y estado del rescate V4, referencias, fases, dependencias, aptitud y trabajo restante. | Autorización automática de código, publicación, piloto, backend o V5. |
...
| Seguimiento V4 S09–S12 | Fase iniciada parcialmente | Contexto/gate S09–S11, borradores/guard S12, adherencia sin interpretación, EA crudo/no interpretado, PROMs manuales/no interpretadas y tarjeta de revisión de solo lectura del último persistido exacto por paciente/línea integrados. La tarjeta no lee dirty inputs, no escribe y no es un registro final/firmado. `PATIENT_NOT_FOUND`, `PATIENT_MISMATCH` y la transición de la misma identidad `blocked`→`active` se demostraron mediante controles soportados; la QA visible mantuvo consola y `pageerror` a cero. Siguen pendientes interpretación clínica de adherencia/EA/PROMs, cálculo DLQI, umbrales/clasificación/alertas/recomendaciones/tendencias, causalidad y decisiones de EA, cambios terapéuticos, outputs de Seguimiento, dashboards y cierre clínico completo. S09–S12 solo están demostrados para este alcance parcial; no hay guard S12 transversal fuera de Seguimiento. Los módulos legacy, causalidad, outputs y dashboards permanecen inertes. |
| Dashboards V4 | Pendientes posteriores | No están reconstruidos contra la fuente canónica. |
| Publicación, release y freeze V4 | Pendientes separados | El último HEAD funcional reconciliado no está publicado ni congelado. La fuente Pages y la candidata permanecen en `2ee3b347...` mediante PR #86; PR #88–#132 no están acreditados en Pages y el estado actual no está acreditado como demo-ready, piloto ni producción. |
