...
| Rama de rescate integrada | `origin/rescue/farmacia-v4`. Último HEAD funcional reconciliado: `969af261b8917765d13c447f09537cedb5dcce28` (PR #136 / issue #135, completados). PR #134 / issue #133 completaron antes una reconciliación exclusivamente documental en `7a9287129ace6b120430c24bbcb36ebd6f73c33f`; no constituyó un HEAD funcional nuevo. |
...
| Rama de trabajo de este índice | `work/docs/fh-post-pr136-confirmed-visit-reconciliation-v4-20260726` |
...
| `origin/rescue/farmacia-v4` | Integración viva del rescate V4. Último HEAD funcional reconciliado: `969af261b8917765d13c447f09537cedb5dcce28` (PR #136 / issue #135, completados). PR #134 / issue #133 fue la reconciliación exclusivamente documental `7a9287129ace6b120430c24bbcb36ebd6f73c33f`, no un HEAD funcional nuevo. | S01–S08 cerrados en su alcance técnico previo. PR #114–#132 integran el alcance parcial de Seguimiento hasta revisión del último borrador v4 persistido. PR #136 añade confirmación profesional explícita de visita demo únicamente desde el último borrador limpio/persistido exacto de la línea activa, con fecha y profesional FH demo; store estricto `farmaciaDemo.followupConfirmedVisits.v1`, registros append-only inmutables en alcance, dedupe por `patient_id + line_id + source_draft_saved_at`, nuevo `saved_at` habilitando otra visita, UUID v4 criptográfico, mapas seguros, rollback/verificación exacta e historial/auditoría aislados. Borrador v4 y multitratamiento no cambian. S09–S12, identidad, recarga y fallos de storage se demostraron por controles soportados con consola/`pageerror` a cero. | Estado publicado, demo-ready, piloto, producción, datos reales, backend o V5. La visita demo no es firma clínica, validez legal ni registro productivo. Siguen pendientes outputs JARA/TXT/CSV/Excel; interpretación de adherencia/EA/PROMs; DLQI; causalidad Naranjo/Karch-Lasagna; fármaco sospechoso/corrección; recomendaciones/alertas/umbrales/tendencias; switch/add-on/optimización/pausa/suspensión/reanudación/finalización y movimientos; dashboards; cierre completo de Seguimiento/S09–S12; y guard S12 transversal fuera de Seguimiento. Legacy, causalidad, movimientos, outputs y dashboards permanecen inertes. |
...
| [`docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md`](/docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md) | Plan vivo reconciliado hasta PR #136 | Gobierno y estado del rescate V4, referencias, fases, dependencias, aptitud y trabajo restante. | Autorización automática de código, publicación, piloto, backend o V5. |
...
| Seguimiento V4 S09–S12 | Fase iniciada parcialmente | Además del contexto/gate, borradores, capturas no interpretadas y revisión persistida, PR #136 integra confirmación profesional explícita de una visita demo desde el último borrador v4 limpio y persistido exacto de la línea activa. Exige fecha de visita y profesional FH demo; conserva borrador y multitratamiento; aísla historial/auditoría por paciente/línea; y demuestra por controles soportados S09–S12, errores de identidad, transición `blocked`→`active`, recarga y fallos de storage con consola/`pageerror` a cero. El registro demo no es firma clínica, validez legal ni registro productivo. Siguen pendientes outputs JARA/TXT/CSV/Excel de Seguimiento, interpretación de adherencia/EA/PROMs, DLQI, causalidad, sospecha/corrección, recomendaciones/alertas/umbrales/tendencias, decisiones y movimientos terapéuticos, dashboards, cierre completo de Seguimiento/S09–S12 y guard S12 transversal fuera de Seguimiento. Legacy, causalidad, movimientos, outputs y dashboards permanecen inertes. |
| Dashboards V4 | Pendientes posteriores | No están reconstruidos contra la fuente canónica. |
| Publicación, release y freeze V4 | Pendientes separados | El último HEAD funcional reconciliado no está publicado ni congelado. La fuente Pages y la candidata permanecen en `2ee3b347...` mediante PR #86; PR #88–#136 no están acreditados en Pages y el estado actual no está acreditado como demo-ready, piloto ni producción. |
