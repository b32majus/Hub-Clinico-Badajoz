...
| Rama de rescate integrada | `origin/rescue/farmacia-v4`. Último HEAD funcional reconciliado: `1d4a68463942518a1d7e38d8a5d2db15da2e6ff7` (PR #128 / issue #127, completados). PR #126 / issue #125 completaron antes una reconciliación exclusivamente documental en `332412d39e32db63b28507fb4205e659ae70a929`; no constituyó un HEAD funcional nuevo. |
...
| Rama de trabajo de este índice | `work/docs/fh-post-pr128-proms-reconciliation-v4-20260726` |
...
| `origin/rescue/farmacia-v4` | Integración viva del rescate V4. Último HEAD funcional reconciliado: `1d4a68463942518a1d7e38d8a5d2db15da2e6ff7` (PR #128 / issue #127, completados). PR #126 / issue #125 fue la reconciliación documental posterior a PR #124 en `332412d39e32db63b28507fb4205e659ae70a929`, no un HEAD funcional nuevo. | S01–S08 cerrados en su alcance técnico previo; PR #114 integra contexto/gate, PR #116 borradores/guard S12, PR #120 adherencia no interpretada, PR #124 EA crudo/no interpretado y PR #128 captura manual explícita de `proms_collected`, `dlqi_total`, `eva_dolor` y `eva_prurito`. Los valores PROMs se introducen manualmente, sin cálculo ni interpretación; cero es válido y la ausencia permanece vacía. Estados: `PROMS_EMPTY`, `PROMS_NOT_RECORDED`, `PROMS_NO_COLLECTION`, `PROMS_RECORDED_INCOMPLETE` y `PROMS_RECORDED_UNINTERPRETED`. El store `farmaciaDemo.followupDrafts.v4` tiene precedencia v4, migraciones no destructivas v3→v4, v2→v4 y v1→v4, lectura fail-closed ante contenido corrupto/incompatible y escritura que, al fallar, conserva UI, baseline, dirty state y store previo. Mantiene aislamiento `patient_id + line_id`; dirty state y guard S12 incluyen notas, adherencia, EA y PROMs. Soporta `PATIENT_NOT_FOUND`, `PATIENT_MISMATCH` y la transición de la misma identidad `blocked`→`active`; QA visible por controles soportados con consola y `pageerror` a cero. | Estado publicado, demo-ready, piloto, producción, datos reales, backend o V5; interpretación clínica de adherencia/PROMs, cálculo DLQI, umbrales, clasificación, alertas o recomendaciones; causalidad de EA, fármaco sospechoso, corrección aplicada o decisiones terapéuticas; optimización/cambio/reducción de dosis/suspensión; outputs JARA/CSV/Excel de Seguimiento, dashboards canónicos, cierre clínico completo de Seguimiento/S09–S12 o guard S12 transversal. Los módulos legacy `modSeguimientoProms` y sus outputs permanecen inertes. |
...
| [`docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md`](/docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md) | Plan vivo reconciliado hasta PR #128 | Gobierno y estado del rescate V4, referencias, fases, dependencias, aptitud y trabajo restante. | Autorización automática de código, publicación, piloto, backend o V5. |
...
| Seguimiento V4 S09–S12 | Fase iniciada parcialmente | Contexto/gate S09–S11, borradores/guard S12, adherencia sin interpretación, EA crudo/no interpretado y captura PROMs manual/no interpretada integrados. `PATIENT_NOT_FOUND`, `PATIENT_MISMATCH` y la transición de la misma identidad `blocked`→`active` se demostraron mediante controles soportados; la QA visible mantuvo consola y `pageerror` a cero. Siguen pendientes interpretación clínica de adherencia/PROMs, cálculo DLQI, umbrales/clasificación/alertas/recomendaciones, causalidad y decisiones de EA, cambios terapéuticos, outputs de Seguimiento, dashboards y cierre clínico completo. S09–S12 solo están demostrados para este alcance parcial; no hay guard S12 transversal fuera de Seguimiento. `modSeguimientoProms` y sus outputs legacy permanecen inertes. |
| Dashboards V4 | Pendientes posteriores | No están reconstruidos contra la fuente canónica. |
| Publicación, release y freeze V4 | Pendientes separados | El último HEAD funcional reconciliado no está publicado ni congelado. La fuente Pages y la candidata permanecen en `2ee3b347...` mediante PR #86; PR #88–#128 no están acreditados en Pages y el estado actual no está acreditado como demo-ready, piloto ni producción. |
...
*Índice generado en el contexto de `WO-DOC-INDEX-CONTROL-PLANE-POST-SES-01-20260713`, reconciliado previamente hasta PR #106 por `WO-DOC-FH-RESCUE-STATUS-RECONCILIATION-V4-01`, actualizado hasta PR #110 por `WO-DOC-FH-POST-PR110-STATUS-RECONCILIATION-V4-01`, reconciliado hasta PR #116 por `WO-DOC-FH-FOLLOWUP-POST-PR114-PR116-RECONCILIATION-V4-01` (issue #117), hasta PR #120 por `WO-DOC-FH-POST-PR120-ADHERENCE-RECONCILIATION-V4-01` (PR #122 / issue #121, completados), hasta PR #124 por `WO-DOC-FH-POST-PR124-AE-RECONCILIATION-V4-01` (PR #126 / issue #125, completados) y hasta PR #128 por `WO-DOC-FH-POST-PR128-PROMS-RECONCILIATION-V4-01` (issue #129, `READY_FOR_CORA_REVIEW`, sin PR ni merge). No autoriza cambios de código, publicación, merges ni integraciones.*
