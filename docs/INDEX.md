...
| Rama de rescate integrada | `origin/rescue/farmacia-v4`. Último HEAD funcional reconciliado: `cbc36870ddb84d2085365e5817f4ef94cf941fce` (PR #124). Entre PR #120 y ese HEAD, PR #122 integró únicamente la reconciliación documental `0f9aa595dc6e93fb92b1d4a8095888f73e05c05d`; no añadió funcionalidad clínica. |
...
| Rama de trabajo de este índice | `work/docs/fh-post-pr124-ae-reconciliation-v4-20260726` |
...
| `origin/rescue/farmacia-v4` | Integración viva del rescate V4. Último HEAD funcional reconciliado: `cbc36870ddb84d2085365e5817f4ef94cf941fce` (PR #124). PR #122 fue una reconciliación documental intermedia en `0f9aa595dc6e93fb92b1d4a8095888f73e05c05d`, no un HEAD funcional nuevo. | S01–S08 cerrados en su alcance técnico previo; PR #114 integra contexto canónico y gate S09–S11 sobre `patient_id + line_id`; PR #116 integra borradores por línea y guard S12; PR #120 integra adherencia estructurada sin interpretación; PR #124 integra captura de borrador de EA crudo/no interpretado (`ae_present`, `ae_description`, `ae_severity`, `ae_resolution`) con estados `AE_EMPTY`, `AE_NOT_RECORDED`, `AE_NO_EVENT`, `AE_PRESENT_INCOMPLETE` y `AE_PRESENT_COMPLETE_UNINTERPRETED`. El store `farmaciaDemo.followupDrafts.v3` tiene precedencia v3, migraciones no destructivas v2→v3 y v1→v3, lectura fail-closed, escritura que conserva UI, baseline, dirty state y store previo al fallar, aislamiento `patient_id + line_id`, y dirty state conjunto de notas/adherencia/EA con guard S12. Si `ae_present != si`, los detalles se limpian, deshabilitan y no persisten. | Estado publicado, demo-ready, piloto, producción, datos reales, backend o V5; interpretación clínica de adherencia, causalidad de EA, fármaco sospechoso, corrección/recomendación/decisión terapéutica aplicada, optimización/cambio/reducción de dosis/suspensión, PROMs (incluidos DLQI/EVA), outputs JARA/CSV/Excel de Seguimiento, dashboards canónicos, cierre clínico completo de Seguimiento/S09–S12 o guard S12 transversal. |
...
| [`docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md`](/docs/ops/PLAN_MAESTRO_RESCATE_FARMACIA_V4_20260724.md) | Plan vivo reconciliado hasta PR #124 | Gobierno y estado del rescate V4, referencias, fases, dependencias, aptitud y trabajo restante. | Autorización automática de código, publicación, piloto, backend o V5. |
...
| Seguimiento V4 S09–S12 | Fase iniciada parcialmente | Contexto/gate S09–S11, borradores/guard S12, adherencia sin interpretación y captura cruda/no interpretada de EA integrados. La búsqueda CIP inválida (`PATIENT_NOT_FOUND`) y `PATIENT_MISMATCH` se demostraron por controles visibles, incluida importación XLSX sintética para conflicto de identidad: cancelar conserva contexto y cambios no guardados; aceptar descarta solo esos cambios; volver al CIP original restaura únicamente el último borrador persistido. Siguen pendientes la interpretación clínica y el cierre clínico completo. S09–S12 solo están demostrados para contexto canónico, borradores, adherencia y EA crudo; no hay guard S12 transversal demostrado. |
| Dashboards V4 | Pendientes posteriores | No están reconstruidos contra la fuente canónica. |
| Publicación, release y freeze V4 | Pendientes separados | El último HEAD funcional reconciliado no está publicado ni congelado. La fuente Pages y la candidata permanecen en `2ee3b347...` mediante PR #86; PR #88–#124 no están acreditados en Pages y el estado actual no está acreditado como demo-ready ni como apto para piloto. |
...
*Índice generado en el contexto de `WO-DOC-INDEX-CONTROL-PLANE-POST-SES-01-20260713`, reconciliado previamente hasta PR #106 por `WO-DOC-FH-RESCUE-STATUS-RECONCILIATION-V4-01`, actualizado hasta PR #110 por `WO-DOC-FH-POST-PR110-STATUS-RECONCILIATION-V4-01`, reconciliado hasta PR #116 por `WO-DOC-FH-FOLLOWUP-POST-PR114-PR116-RECONCILIATION-V4-01` (issue #117), hasta PR #120 por `WO-DOC-FH-POST-PR120-ADHERENCE-RECONCILIATION-V4-01` (PR #122 / issue #121, completados) y hasta PR #124 por `WO-DOC-FH-POST-PR124-AE-RECONCILIATION-V4-01` (issue #125, `READY_FOR_CORA_REVIEW`, aún sin PR ni merge). No autoriza cambios de código, publicación, merges ni integraciones.*
