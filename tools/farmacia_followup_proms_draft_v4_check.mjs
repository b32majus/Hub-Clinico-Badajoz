...
test('legacy PROM remains inert and canonical outputs do not connect to PROM draft DOM', () => { assert.match(contextSource, /section\.inert = true/); assert.match(contextSource, /__farmaciaFollowupOutputsV4Installed/); assert.doesNotMatch(contextSource, /function protect\(|__farmaciaFollowupBlockedV4/); assert.doesNotMatch(source, /fhSegPromsExpanded|fhSegDlqiQuestions|fhSegDlqiInterp|FarmaciaExcelRowExport|copyTextToClipboard/); });
