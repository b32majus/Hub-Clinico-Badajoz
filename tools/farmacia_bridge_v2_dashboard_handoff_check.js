...
test('trimmed search context resolves explicit identifier', () => assert(H.validatePayload(payload({ search_context: { identifier_system: ' urn:cip:demo ', identifier_value: ' SAME-VALUE ' }, quick_view: view({ identifiers: [{ identifier_system: 'urn:cip:demo', identifier_value: 'SAME-VALUE' }] }) }))));
...
test('incomplete search context rejected', () => rejects(() => H.validatePayload(payload({ search_context: { identifier_system: '', identifier_value: 'x' } })), 'HANDOFF_SEARCH_CONTEXT_INVALID'));
