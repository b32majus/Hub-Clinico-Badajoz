...
const input = {
  ...
  filas_fh: [
    { visit_id: 'V2', fecha: '2026-07-02', line_id: 'L2', tratamiento: 0, estado: false, evaluated: true, dispensed: false, dispensed_amount: 'No recogido', prom: { tipo_prom: 'DLQI', valor: 0 }, evento_adverso: { ea_id: 'EA-1', tipo: 'EA' } },
    { visit_id: 'V1', fecha: '2026-07-01', line_id: 'L1', tratamiento: 'Tratamiento A', estado_linea: 'activo', evaluated: true, dispensed: true, dispensed_amount: 0, prom: { tipo_prom: 'DLQI', valor: 0 }, evento_adverso: { ea_id: 'EA-2', tipo: 'EA 2' } },
...
check(result.proms.length === 7 && result.visitas_fh[0].proms.length === 2 && result.visitas_fh[0].proms[0].valor === 11 && result.visitas_fh[0].proms[1].tipo_prom === 'HAQ', 'PROM superior se deduplica y adjunta una vez antes del PROM de fila coincidente');
...
check(result.visitas_fh[0].lineas[0].dispensed_amount === 0 && result.visitas_fh[4].visit_id === undefined && result.visitas_fh[4].lineas[0].line_id === undefined, 'cero permanece cero y no se inventan visit_id ni line_id ausentes');
...
check(result.visitas_fh.find((visit) => visit.visit_id === 'V2').lineas[0].dispensed_amount === 'No recogido' && result.visitas_fh.find((visit) => visit.visit_id === 'V2').lineas[0].tratamiento === 0 && result.visitas_fh.find((visit) => visit.visit_id === 'V2').lineas[0].estado === false, '“No recogido”, tratamiento cero y estado false se preservan sin coerción');
...
check(fh004.filas_fh.length === 2 && fh004.filas_fh[0].visit_id === fh004.filas_fh[1].visit_id && fh004.filas_fh[0].prom.tipo_prom === fh004.filas_fh[1].prom.tipo_prom && fh004.filas_fh[0].evento_adverso.ea_id === fh004.filas_fh[1].evento_adverso.ea_id, 'fixture FH-004 contiene dos filas fuente y comunes PROM/EA duplicados');
check(fixtureResult.visitas_fh.length === 1 && fixtureResult.visitas_fh[0].lineas.length === 2 && fixtureResult.visitas_fh[0].proms.length === 1 && fixtureResult.visitas_fh[0].eventos_adversos.length === 1 && fixtureResult.eventos_adversos.length === 1, 'fixture FH-004 normaliza a una visita, dos líneas, un PROM y un EA visible');
...
check(fixtureResult.visitas_fh[0].lineas.filter((line) => line.dispensed === true).length === 1 && fixtureResult.visitas_fh[0].lineas[0].dispensed_amount === 'No recogido', 'fixture conserva exclusivamente la dispensación explícita y “No recogido” literal');
