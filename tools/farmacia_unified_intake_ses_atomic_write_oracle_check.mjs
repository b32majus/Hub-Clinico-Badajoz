...
function faultedValidationHtml(root, fault) {
  let html = readFileSync(path.join(root, 'farmacia_validacion.html'), 'utf8');
  if (!fault) return html;
  if (fault === 'missing-label') return html.replace(/^[^\n]*id="fhDermaSesProgramLabel"[^\n]*\r?\n/m, '');
  if (fault === 'missing-code') return html.replace(/^[^\n]*id="fhDermaSesProgramCode"[^\n]*\r?\n/m, '');
  if (fault === 'missing-pathology') return html.replace(/^[^\n]*id="fhDermaPatologia"[^\n]*\r?\n/m, '');
  if (fault === 'drift-option') return html.replace('<option>Hidradenitis supurativa</option>', '<option>HIDRADENITIS SUPURATIVA</option>');
...
const server = createServer((request, response) => {
  ...
  try {
    ...
    if (path.basename(file) === 'farmacia_validacion.html' && activeFault) {
      response.end(faultedValidationHtml(ROOT, activeFault));
    } else {
...
async function selectedPage(fault = null) {
...
async function valueOrNull(page, selector) {
  const loc = page.locator(selector);
  return await loc.count() ? await loc.inputValue() : null;
}
async function assertZeroMutation(page, expectedPathology = '') {
  assert.equal(await valueOrNull(page, '#fhDermaPatologia'), expectedPathology, 'visible pathology mutated on failed atomic preflight');
  const code = await valueOrNull(page, `#${SES_CODE_ID}`);
  const label = await valueOrNull(page, `#${SES_LABEL_ID}`);
  if (code !== null) assert.equal(code, '', 'SES code partially written');
  if (label !== null) assert.equal(label, '', 'SES label partially written');
}
async function runFault(fault, expectedPathology = '') {
  const { context, page } = await selectedPage(fault);
  try {
    if (await page.locator('#fhDermaPatologia').count()) await page.locator('#fhDermaPatologia').selectOption(expectedPathology);
    await preview(page, eordenRaw('SES_HS', 'HIDRADENITIS SUPURATIVA', 'HIDRADENITIS SUPURATIVA'));
    await confirmSes(page);
    await assertZeroMutation(page, expectedPathology);
...
try {
  {
    ...
    await preview(page, eordenRaw('SES_HS', 'HIDRADENITIS SUPURATIVA', 'HIDRADENITIS SUPURATIVA'));
    await confirmSes(page);
    assert.equal(await page.locator('#fhDermaPatologia').inputValue(), 'Hidradenitis supurativa');
    assert.equal(await page.locator(`#${SES_CODE_ID}`).inputValue(), 'SES_HS');
    assert.equal(await page.locator(`#${SES_LABEL_ID}`).inputValue(), 'HIDRADENITIS SUPURATIVA');
    assert.deepEqual(page.__errors, []);
    await context.close();
    console.log('OK 1 positive SES_HS write-set'); passed++;
  }
  ...
  console.error(`T9 SES ATOMIC WRITE ORACLE FAIL after ${passed} passed scenario groups: ${error?.stack || error}`);
  process.exitCode = 1;
} finally {
  await browser.close();
