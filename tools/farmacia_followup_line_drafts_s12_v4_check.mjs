...
test('Hub and pre-Hub use the same independent v3 store', () => assert.equal(drafts.STORE_KEY, 'farmaciaDemo.followupDrafts.v3'));
...
test('draft schema contains only identity, notes, adherence, raw AE and audit strings', () => { assert.deepEqual(Object.keys(object()).sort(), ['draft_id', 'kind', 'line_id', 'notes', 'patient_id', 'mg1', 'mg2', 'mg3', 'mg4', 'ae_present', 'ae_description', 'ae_severity', 'ae_resolution', 'saved_at', 'saved_by_demo'].sort()); assert.doesNotMatch(draftSource, /drug_name|dose_text|route|pauta|therapy/); });
