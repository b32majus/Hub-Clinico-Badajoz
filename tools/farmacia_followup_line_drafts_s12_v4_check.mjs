...
test('Hub and pre-Hub use the same independent v4 store', () => assert.equal(drafts.STORE_KEY, 'farmaciaDemo.followupDrafts.v4'));
...
test('draft schema contains only identity, notes, adherence, raw AE, raw PROM and audit fields', () => { assert.deepEqual(Object.keys(object()).sort(), ['draft_id', 'kind', 'line_id', 'notes', 'patient_id', 'mg1', 'mg2', 'mg3', 'mg4', 'ae_present', 'ae_description', 'ae_severity', 'ae_resolution', 'proms_collected', 'dlqi_total', 'eva_dolor', 'eva_prurito', 'saved_at', 'saved_by_demo'].sort()); assert.doesNotMatch(draftSource, /drug_name|dose_text|route|pauta|therapy/); });
test('outputs remain UI guarded without monkey-patching global helpers', () => { assert.match(contextSource, /__farmaciaFollowupOutputsV4Installed/); assert.doesNotMatch(contextSource, /target\[key\]\s*=|__farmaciaFollowupBlockedV4|function protect\(/); });
...
test('search and line selection decide on requested identity before retaining only the emitted draft partition', () => { assert.match(contextSource, /var partition = draftPartitionIdentity \|\| result;[\s\S]*var detail = \{ ok: !!result\.ok, code: result\.code, patient_id: partition\.patient_id \|\| '', line_id: partition\.line_id \|\| '', status:/); assert.match(contextSource, /function searchCip[\s\S]*var identity = \{ cip: cip,[\s\S]*beforeContextChange\(\{[\s\S]*next: identity[\s\S]*if \(decision === 'cancel'[\s\S]*var draftPartition = retainedDraftPartition\(previousContext, resolved\);/); assert.match(contextSource, /function selectLine[\s\S]*identity\.line_id = text\(lineId\);[\s\S]*beforeContextChange\(\{[\s\S]*next: identity[\s\S]*if \(decision === 'cancel'[\s\S]*var draftPartition = retainedDraftPartition\(previousContext, candidate\);/); assert.doesNotMatch(contextSource, /next: \{ patient_id: candidate\.patient_id|next: draftPartition \|\| identity|decisionIdentity/); });
