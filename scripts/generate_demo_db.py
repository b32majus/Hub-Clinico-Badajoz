...
def run_post_generation_validations(
    ...
    for sheet, rows in rows_by_sheet.items():
        if not rows:
            continue
        pid = rows[0].get("ID_Paciente", "")
        patient_visits[pid] = len(rows)
    all_multi = all(v >= 3 for v in patient_visits.values())
    checks.append(ValidationCheck("cada paciente tiene varias visitas", all_multi, str(patient_visits)))
    ...
    for sheet, rows in rows_by_sheet.items():
        parsed_dates = [parse_iso_date(r.get("Fecha_Visita", "")) for r in rows]
